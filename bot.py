...
def get_estimate(amount_usd: float, pay_currency: str) -> float | None:
    """Get estimated crypto amount from NowPayments."""
...
def get_min_amount(pay_currency: str, is_fixed_rate: bool = True) -> float:
    """Return the minimum amount in USD for the selected currency."""
...
def coin_keyboard(last_coin: str | None = None) -> InlineKeyboardMarkup:
    buttons = []
    for code, name in COINS.items():
        prefix = "✅ " if code == last_coin else ""
        buttons.append(
            [InlineKeyboardButton(f"{prefix}{name}", callback_data=f"coin_{code}")]
        )
...
async def receive_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if amount < 5:
            await update.message.reply_text(
                "Minimum deposit is **$5**. Please try again.",
                parse_mode="Markdown",
            )
            return AMOUNT
        ...
        await update.message.reply_text(
            "Please enter a valid number (e.g. 10 or 25.5)"
        )
        return AMOUNT
...
async def receive_coin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "cancel_topup":
        await query.message.reply_text(
            "Top-up cancelled.", reply_markup=main_keyboard()
        )
    ...
    if estimated:
        estimate_text = (
            f"≈ **{estimated:.8f} {coin_code.upper()}**".rstrip("0").rstrip(".")
        )
    else:
    ...
    confirm_keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "✅ Confirm & Get Address",
                    callback_data=f"confirm_{coin_code}",
                ),
...
async def confirm_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        display_amount = f"{float(pay_amount):.8f}".rstrip("0").rstrip(".")

        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute(
                """
                INSERT OR REPLACE INTO pending_payments
                (payment_id, user_id, amount) VALUES (?, ?, ?)
                """,
        ...
        await query.message.reply_photo(
            photo=qr_url, caption=text, parse_mode="Markdown"
        )
        await query.message.reply_text(
            "You can return to the main menu.", reply_markup=main_keyboard()
        )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "💰 My Balance":
        ...
        await update.message.reply_text("📦 Your orders will appear here soon…")
    elif text == "🆘 Support":
        await update.message.reply_text(
            "🆘 Customer Support\n\n"
...
async def main():
    ...
    if not NOWPAYMENTS_API_KEY:
        logger.warning("NOWPAYMENTS_API_KEY is not set – payments will fail")
